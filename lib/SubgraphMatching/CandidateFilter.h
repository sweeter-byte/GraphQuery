...
namespace GraphLib::SubgraphMatching {
    ...
    bool CandidateSpace::RefineCS(){
        //std::cout << "Entry RefineCS " << std::endl;
        Timer VertexSafetimer;
        Timer NeighborSafetimer;
        ...
        while (queue_pop_count < maximum_queue_cnt) {
            ...
            Timer NeighborSafetimer_first;
            NeighborSafetimer_first.Start();
            ...
            NeighborSafetimer_first.Stop();
            int loop_ub = candidate_set_[cur].size();
            std::vector<char> flags(loop_ub, 'T');
            #pragma omp parallel for proc_bind(close) num_threads(GraphLib::num_threads)
            for (int i = 0; i < loop_ub; i++) {
                ...
                // 邻居安全
                NeighborSafetimer.Start();
                if (valid) valid = NeighborFilter(cur, cand);
                NeighborSafetimer.Stop();
                NeighborSafetimer.Add(NeighborSafetimer_first);
            ...
            std::vector<int> new_canddidates;
            for (int i = 0; i < loop_ub; i++) {
                if (flags[i] == 'T') {
                    new_canddidates.push_back(candidate_set_[cur][i]);
                }
            }
            candidate_set_[cur] = new_canddidates;
        ...
        std::cout << "[Perf] VertexSafeTime: " << VertexSafetimer.GetTime() << "ms" << std::endl;
        std::cout << "[Perf] NeighborSafeTime: " << NeighborSafetimer.GetTime() << "ms" << std::endl;
        std::cout << "[Perf] TriangleSafetyTime: " << triangle_safety_timer_.GetTime() << "ms" << std::endl;
        std::cout << "[Perf] FourCycleSafetyTime: " << four_cycle_safety_timer_.GetTime() << "ms" << std::endl;
        return true;
    }
    ...
    bool CandidateSpace::CheckNeighborSafety(int cur, int cand) {
        std::vector<int> local_label_frequency(data_->GetNumLabels(), 0);
        for (int d_neighbor : data_->GetNeighbors(cand)) {
            if (in_neighbor_cs[d_neighbor]) {
                local_label_frequency[data_->GetVertexLabel(d_neighbor)]++;
            }
        }
        bool valid = true;
        for (int l = 0; l < data_->GetNumLabels(); ++l) {
            if (neighbor_label_frequency[l] - local_label_frequency[l] > 0) {
                valid = false;
                break;
            }
        }
    ...
    inline bool CandidateSpace::TriangleSafety(int query_edge_id, int data_edge_id) {
        Timer TriangleSafetytimer;
        TriangleSafetytimer.Start();
        ...
        TriangleSafetytimer.Stop();
        //std::cout << "[Perf] TriangleSaftyTime:" << TriangleSafetytimer.GetTime() << "ms" << std::endl;
        return true;
    };

    bool CandidateSpace::FourCycleSafety(int query_edge_id, int data_edge_id) {
        Timer FourCycleSafetytimer;
        FourCycleSafetytimer.Start();
        ...
        //#pragma omp parallel for shared(failed) num_threads(GraphLib::num_threads)
        for (int i = 0; i < query_cycles.size(); ++i) {
            if (failed) break;
    
            auto &q_info = query_cycles[i];
            bool cycle_matched = false;
    
            const int data_cycle_size = data_cycles.size();
            for (int j = 0; j < data_cycle_size && !cycle_matched; ++j) {
                auto &d_info = data_cycles[j];
                bool validity = true;
    
                // 边约束检查（短路优化）
                validity = validity && BitsetEdgeCS[get<1>(q_info.edges)][get<1>(d_info.edges)];
                validity = validity && BitsetEdgeCS[get<2>(q_info.edges)][get<2>(d_info.edges)];
                validity = validity && BitsetEdgeCS[get<3>(q_info.edges)][get<3>(d_info.edges)];
                if (!validity) continue;
    
                // 对角线约束检查
                if (validity && get<0>(q_info.diags) != -1)
                    validity = validity && EdgeCandidacy(get<0>(q_info.diags), get<0>(d_info.diags));
                
                if (validity && get<1>(q_info.diags) != -1)
                    validity = validity && EdgeCandidacy(get<1>(q_info.diags), get<1>(d_info.diags));
    
                cycle_matched = validity;
            }
        ...
        FourCycleSafetytimer.Stop();
        //std::cout << "[Perf] FourCycleSafetyTime: " << FourCycleSafetytimer.GetTime() << "ms" << std::endl;
        return !failed;
    }
