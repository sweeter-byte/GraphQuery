...
E3: Overall M3 EPS improvement (baseline vs optimized pipeline).
E5: E3 results grouped by query size |V_q|.
E6: E3 results grouped by dataset.

All three share the same data collection — grouping is done at analysis time.
...
def _run_m3(engine, data_graph_path, tmp_query, max_emb, time_limit):
    """Execute M3 and return (eps, embeddings, time_s)."""
    try:
...
def main():
    ...
    csv_out = ExperimentCSV(
        ...
        columns=[
            ...
            # Deltas
            "plan_speedup", "eps_improvement",
        ],
        metadata={"experiment": "E3+E5+E6"},
    )
    ...
    try:
        for ds in args.datasets:
            ...
            for q in queries:
                ...
                # --- M3 execution for both top-1 orders ---
                ...
                eps_impr = (opt_eps - bl_eps) / bl_eps if bl_eps > 0 else 0.0
