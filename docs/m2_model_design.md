...
给定一个 (data_graph, query_graph) 对和 N 个候选匹配序列 $S = \{s_1, s_2, \dots, s_N\}$（M1 去重后约 50–80 个），**选出执行性能最优的序列**（最高 EPS 或最低枚举时间）。

### 1.1 当前基线：手工加权求和

当前 M2 使用线性代价模型对每个序列 $O$ 打分：
...
用 ML 模型替代上述手工加权求和，使最优序列的选中率（Top-1 Accuracy）显著提升。
...
这是模型的**主输入**，反映了按该序列逐步扩展匹配时的搜索空间增长趋势。
